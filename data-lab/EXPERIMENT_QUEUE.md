...
DL-002 is complete with a `Revise` decision. It found zero verified comparison
dishes, 16.67% claimed current-menu coverage, 13.49% claimed current-menu plus
matched-Management-photo coverage, and zero national content coverage in the
hidden active holdout. DL-013 then found that DoorDash's standard APIs are not
a national acquisition connector: schema quality and national geography
passed, but a marketplace-wide read grant, downstream rights, and
...
|---|---|---|---|
| DL-007 | Can SeeFood's own targeted Management and Customer prompts close the comparison gap cheaply enough to scale nationally? | Stage 4: canonical gold predicate, one-shot terminal review, trusted traffic eligibility, same-snapshot selection ledger, and at most 100 label-plus-image candidates from the behavioral intersection; treatment stays off | Stage 3 Revise; Push 4 required |
...
| Complete | DL-002 | A forced-read-only snapshot, an independent local identity/status frame, and frozen cohorts will reveal the real largest gap and separate claimed from verifiable comparisons. | 396 Temecula identities; 108 active hidden national identities plus 12 status sentinels; 320-photo evidence packet | Established the baseline and exposed the true gaps | Revise: 0/21 comparisons verified; denominator and ladder corrected |
| Complete | DL-013 | A DoorDash-authorized catalog/data path can add at least 20 percentage points of national current-menu-plus-strong-Management-photo coverage. | Public documentary gate plus one bounded sitemap-index request; no store crawl, account, outreach, or hidden-holdout access | Technical schema and nationwide footprint passed; marketplace-wide access, rights, and representative yield did not | Revise standard connector; preserve bespoke and scraping opportunities |
| 1 | DL-007 | Targeted Management and Customer contribution prompts close the final comparison gap more cheaply than another automatic source. | Stage 4 must align terminal comparison creation with the gold contract, make review one-shot, prevent contradictory receipts, verify traffic eligibility, reconcile the six-row selection correction, and blind-audit at most 100 behavioral-intersection rows with labels and images; no treatment or DataLab production access | Tests the Customer-supply frontier before adding more Management-only pipes | Stage 3 Revise: 4,036 behavioral candidates; 0 gold; 99/100 visually plausible, item match unauditable |
| 2 | DL-012 | A transaction-triggered Tattle partnership can export rights-valid Customer meal photos bound to exact order lines at nationally material unique-location yield. | Documentary gate first: controller-authorized redacted schema/rights packet; only if it passes, at least 1,000 aggregate already-delivered prompts plus 35 blinded historical photo records across at least 10 locations, including multi-item orders; no DataLab outreach or SMS | Highest-potential Customer channel found; Tattle claims 15,000+ locations and already has meal-photo upload, item context, and APIs, but exact binding, rights, and yield are unknown | Requires explicit authorization to approach controller |
| 3 | DL-014 | If rendered retrieval were authorized and reliable, DoorDash public-store extraction could add at least 20 percentage points of national current-menu-plus-strong-photo coverage. | First use stored replays and existing evidence. A live nationally stratified 30-restaurant/300-image probe requires either written DoorDash consent or a separate founder amendment of the current bot-control boundary; never unbounded | Directly measures the theoretical scraping ceiling without confusing it with the standard API path | High-potential technical hypothesis; execution gate unresolved |
...
accessible recorded locator. It makes a nationally broad Management-source
qualification and the transaction-triggered Customer deal gate more important:
only 68/504 restaurants currently have a claimed current menu plus a matched
Management photo, none in the national active holdout, and verified Customer
coverage is zero. Do not begin a large connector or partnership implementation
until one bounded path proves national reach against the game-changer screen.
...
| DL-002 | Revise / baseline established | Corrected active denominator 504 plus 12 status sentinels. Claimed current menus 84; current-menu attachment candidates 64/60/46 across the upper rungs; six restaurants and 21 dishes claimed comparison-ready; zero verified. Management item-match audit 175/214 (81.78%); no Customer photo was accessible for audit. |
