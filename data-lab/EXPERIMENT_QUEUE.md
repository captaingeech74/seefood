...
DL-002 is active. Stage 1 froze the independently reviewed Temecula development
cohort and hidden national holdout. Stage 2 evidence export and blind baseline
evaluation are the only work now authorized; no source experiment may bypass
them.
...
|---|---|---|---|
| DL-002 | What is the real claimed-versus-verified baseline on frozen Temecula and national cohorts? | 396-record Temecula development cohort; 120-record Guardian-owned national holdout plus 12 alternates; read-only sanitized evidence | Stage 1 accepted; Stage 2 evidence pending |
...
| 0 | DL-002 | A forced-read-only snapshot, an independent local identity/status frame, and frozen cohorts will reveal the real largest gap and separate claimed from verifiable comparisons. | 396 accepted Temecula identities; county 100-record omission challenge; 120 hidden national records plus 12 alternates; SELECT-only evidence export | Required before source experiments | Stage 1 accepted; request Stage 2 only by registered public-ID hashes |
