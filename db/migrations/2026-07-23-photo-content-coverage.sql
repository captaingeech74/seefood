-- Count canonical photo identities once while preserving every legitimate
-- menu-item association captured in photo_menu_item_links.
...
with scoped_entities as (
  ...
  select r.entity_id, p.id, p.photo_author_type,
    coalesce(p.canonical_dish_id::text, case when p.menu_item_id is not null then 'menu-' || p.menu_item_id::text end) as primary_dish_key
  from scoped_restaurants r
...
with menu_counts as (
  ...
  select
    ...
    count(distinct id) as photo_count,
    count(distinct id) filter (where dish_key is not null) as matched_photo_count,
    count(distinct id) filter (where photo_author_type = 'management') as management_photo_count,
    count(distinct id) filter (where photo_author_type = 'customer') as customer_photo_count
  from photo_associations
  group by entity_id
), comparisons as (
  select entity_id, count(*) as comparison_dish_count
...
select
  ...
  coalesce(c.comparison_dish_count, 0) as comparison_dish_count,
  case
    when coalesce(c.comparison_dish_count, 0) >= 1 then 3
...
left join menu_counts m on m.entity_id = e.id
left join photo_counts p on p.entity_id = e.id
left join comparisons c on c.entity_id = e.id;
